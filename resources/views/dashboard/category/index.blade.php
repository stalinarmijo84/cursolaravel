...
@section('content')
<br>
<a class="btn btn-primary mt-3 mb-3" href="{{route('category.create')}}"><i class="fa fa-plus" aria-hidden="true"></i>
  Nuevo</a>
<table id="datatable-responsive" class="table table-striped table-bordered responsive no-wrap" cellspacing="0"
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <a class="btn btn-success" href="{{route('category.show',$item->id)}}">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form id="formDelete" action="{{route('category.destroy',0)}}" data-action="{{route('category.destroy',0)}}"
          method="POST">
          @method('DELETE')
          @csrf
...
<script>
  window.onload = function() {
    $('#deleteModal').on('show.bs.modal', function(event) {
      ...
      var modal = $(this)
      modal.find('.modal-title').text('Borrar Categoria: ' + id)
    })
  };
</script>
@endsection